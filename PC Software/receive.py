...
parser = argparse.ArgumentParser(description='receives the data from a climate logger via serial (https://github.com/martin2250/EnvironmentalLogger)', epilog='to import the data into influxDB, use this command: "influx -host <host> -import -path=<fout>"')
parser.add_argument('port', help='which serial port to use')
parser.add_argument('fout', help='output file name, \'-\' = stdio')
parser.add_argument('-f', '--format', help='output file format', choices=['influx', 'tsv'], default='tsv')
parser.add_argument('--measurement', help='measurement name and tags (only for influx output format)', default='test')
parser.add_argument('--database', help='database (only for influx output format)', default='<DB>')
...
with serial.Serial(args.port, 250000) as port, sys.stdout if args.fout == '-' else open(args.fout, 'w') as fout:
	...
	while True:
		...
		if x[0] == 0xFF:
			...
			print('invalid start sequence, aborting')
			exit(1)
	...
	while pagecounter < pagecount:
		...
		for i in range(samples):
			...
			file_write(fout, date + datetime.timedelta(minutes=i*interval_minutes), temperature, humidity, battery if i == 0 else None)
	file_end(fout)
print('\ndone')
